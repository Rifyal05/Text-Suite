...
def calculate_idf_for_summary(processed_sentences):
    ...
    for word, count in word_in_sentence_count.items():
        idf_scores[word] = math.log((total_sentences + 1) / (count + 1)) + 1
    return idf_scores
...
def calculate_sentence_scores_for_summary(processed_sentences, tf_scores, idf_scores):
    ...
    for i, sentence_tokens in enumerate(processed_sentences):
        ...
        sentence_scores.append(score / (math.sqrt(len(sentence_tokens)) + 1e-6))
...
def extract_keywords_with_rake(text, language='english', max_keywords=10):
    if not text or not text.strip(): return [], "STATUS_INFO: Teks kosong, tidak ada kata kunci untuk diekstrak."
    try:
        current_stopwords_rake = stopwords.words(language.lower())
    except OSError:
        current_stopwords_rake = stopwords.words('english')
    try:
        r = Rake(language=language.lower(), stopwords=current_stopwords_rake, min_length=1, max_length=3)
        r.extract_keywords_from_text(text)
        ranked_phrases = r.get_ranked_phrases_with_scores()
        keywords = [phrase for score, phrase in ranked_phrases[:max_keywords]]
        if not keywords: return [], "STATUS_INFO: Tidak ditemukan kata kunci signifikan dari teks ini."
        return keywords, ""
    except Exception as e:
...
def summarize_with_textrank_sumy(text_input, num_sentences, actual_language_of_text, sastrawi_stemmer_instance_for_sumy_tr, max_keywords=10):
    ...
    if actual_language_of_text.lower() == 'indonesian':
        try:
            ...
            if not indonesian_sentences: return "STATUS_INFO: Tidak dapat memecah teks menjadi kalimat (rule-based).", [], ""
    ...
    try:
        ...
        if not summary_text: return "STATUS_INFO: TextRank tidak menghasilkan ringkasan.", [], ""
...
def get_summary_and_keywords(text_to_process, num_sentences, language, sastrawi_stemmer_instance, max_keywords=10, algo_choice='tfidf'):
    if not text_to_process or not text_to_process.strip():
        return "STATUS_ERROR: Teksnya kosong, Kak...", [], "Teks kosong.", "Teks kosong."
    ...
    if algo_choice.lower() == 'textrank':
        ...
        final_summary_msg = ""
        if summary_output.startswith("STATUS_"):
            final_summary_msg = summary_output
            summary_output = ""
        return summary_output, keywords_output_data, final_summary_msg, kw_err_msg_from_tr_func

    elif algo_choice.lower() == 'tfidf':
        try:
            current_stopwords_lang = language.lower()
            try:
                stop_words_list = set(stopwords.words(current_stopwords_lang))
            except OSError:
            ...
            if language == 'indonesian':
                tambahan_stopwords = [
                    ...
                    'utk', 'ya', 'jd', 'jgn', 'sdh', 'aja', 'n', 't', 'hehe', 'dll', 'dst', 'sbg',
                    'mengatakan', 'menjelaskan', 'menurut', 'pihaknya', 'tersebut', 'yakni', 'yaitu', 'namun', 'demikian'
                ]
                stop_words_list.update(tambahan_stopwords)
            ...
            if not processed_tokens_list or all(not s for s in processed_tokens_list):
                return "STATUS_ERROR: Setelah diproses, teks tidak memiliki kata-kata penting.", [], "Teks tidak memiliki kata penting.", ""
            ...
            if num_to_summarize > 0 and len(scored_original_sentences_with_index) > 0:
                total_sentences = len(scored_original_sentences_with_index)
                
                chunk_size = total_sentences // num_to_summarize if num_to_summarize != 0 else total_sentences
                if chunk_size == 0:
                    chunk_size = 1

                sentence_chunks = []
                for i in range(0, total_sentences, chunk_size):
                    sentence_chunks.append(scored_original_sentences_with_index[i:i + chunk_size])

                for chunk in sentence_chunks:
                    if chunk:
                        chunk.sort(key=lambda x: x[0], reverse=True)
                        top_sentences_data.append(chunk[0])

                top_sentences_data = top_sentences_data[:num_to_summarize]

            top_sentences_data.sort(key=lambda x: x[2])

            top_sentences_list = [sent_data[1] for sent_data in top_sentences_data]
            summary_text_result = _format_summary_into_paragraphs(top_sentences_list)
            
            if not summary_text_result:
                 return "STATUS_INFO: TF-IDF tidak menghasilkan ringkasan.", [], "", ""

            kw_list, kw_err = extract_keywords_with_tfidf(processed_tokens_list, idf_scores, max_keywords)
            
            return summary_text_result, kw_list, "", kw_err

        except ValueError as ve:
            return f"STATUS_ERROR: {str(ve)}", [], str(ve), ""
        except Exception as e_tfidf:
            tb_str = traceback.format_exc()
            return f"STATUS_ERROR: Error internal TF-IDF: {str(e_tfidf)}\n{tb_str}", [], str(e_tfidf), ""

    else:
        err_msg = f"STATUS_ERROR: Algoritma '{algo_choice}' tidak dikenal."
